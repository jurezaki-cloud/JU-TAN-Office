...
def bundled_logo_path() -> Path | None:
    """Dark-on-light horizontal JU-TAN logo for white PDF pages.

    Prefer ``logo_light.png`` (black mark). ``logo.png`` is white-on-transparent
    for dark UI chrome and must not be used on invoice paper.
    """
    names = ("logo_light.png", "logo.png")
    roots: list[Path] = [RESOURCE_DIR, BASE_DIR / "resources"]
