...
def resolve_brand_logo(mode: ThemeMode | None = None) -> Path | None:
    ...
    Use :func:`resolve_brand_logo_for_surface` when the widget background
    contrast differs from the application theme (e.g. always-dark sidebar).
    """
...
def resolve_brand_logo_for_surface(*, dark_surface: bool) -> Path | None:
    """Resolve brand logo for the *actual* surface contrast, not app theme alone.

    The left sidebar stays dark in both LIGHT and DARK application themes, so
    it must use the light-ink (DARK-mode) variant. Login/dialog chrome follows
    the application theme via :func:`resolve_brand_logo`.
    """
    return resolve_brand_logo(ThemeMode.DARK if dark_surface else ThemeMode.LIGHT)
...
class SidebarHeader(QWidget):
    ...
    def refresh_logo(self) -> None:
        """Force reload so surface-aware logo stays correct after theme changes."""
        self._logo_path = None
        self._load_logo()
    ...
    def _load_logo(self) -> None:
        if self._collapsed:
            return
        # Sidebar background is always dark — pick contrast for the surface.
        path = resolve_brand_logo_for_surface(dark_surface=True)
        if path is None:
            self.logo.hide()
            self.brand.show()
        ...
        if (
            ...
            and self.logo.pixmap() is not None
            and not self.logo.pixmap().isNull()
        ):
            return
